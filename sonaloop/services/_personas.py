...
def _validated_persona_patch(persona: dict[str, Any], patch: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if "capabilities" in clean:
        clean["capabilities"] = merge_capabilities(persona.get("capabilities"),  # noqa: F821 (bound)
                                                   clean["capabilities"])
    ...
    for key, value in clean.items():
        if isinstance(value, dict) and isinstance(candidate.get(key), dict):
            ...
            candidate[key] = value
    # Reuse the create-path validator as a whole-profile integrity check.  It
    # catches empty required fields and malformed authored lists before any write.
