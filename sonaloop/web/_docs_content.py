...
PRINCIPLES = [
    ...
    ("syntheses",
     ...
     ("Jeder Report lässt sich direkt als kurze **Stakeholder-PDF** oder als editierbares **PowerPoint** teilen. Vor dem Export entsteht ein zur Methodik passender Präsentationsplan: Entscheidung und nächste Schritte im Hauptteil, visuelle Evidenz wie Screens, Personas und Reaktionsverläufe dazwischen, Details und Quellen im Anhang. Der vollständige Sprechtext, Belege, Einschränkungen und Übergänge landen als echte PowerPoint-Sprechernotizen auf jeder Folie. Workspace-Owner laden unter **Workspace → Vorlagen** einen 16:9-Firmenmaster hoch. Er bestimmt Markenwelt, Layouts, Schriften und Farben – die Dramaturgie und Informationsgestaltung funktionieren auch ohne Master. Downloads verändern den Research-Fortschritt nicht.",
      "Every report can be shared directly as a concise **stakeholder PDF** or editable **PowerPoint**. Before export, Sonaloop creates a methodology-aware presentation plan: decision and next actions in the core story, visual evidence such as screens, personas, and reaction shifts between them, and detail and sources in the appendix. Full talk tracks, evidence, caveats, and transitions become native PowerPoint speaker notes on every slide. Workspace owners upload a 16:9 company master under **Workspace → Templates**. It owns brand, layouts, fonts, and colors, while the story and information design also work without one. Downloads never advance the research run.")),
    ("prototype",
