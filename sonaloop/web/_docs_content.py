...
PRINCIPLES = [
    ...
    ("syntheses",
     ...
     ("Jeder Report lässt sich direkt als kurze **Stakeholder-PDF** oder als editierbares **PowerPoint** teilen. Für zehn Minuten entstehen normalerweise sechs bis acht entscheidungsorientierte Kernfolien: Decision Dashboard, reale Screens, Persona-Sammelfolie, Reaktionsverlauf und ein konkreter editierbarer Änderungsvorschlag statt verteilter Text-Findings. Antwortmatrix, Methodik und Quellen bleiben im kurzen Anhang; Sprechtext, Belege, Einschränkungen und Übergänge landen als echte PowerPoint-Sprechernotizen. Workspace-Owner laden unter **Workspace → Vorlagen** einen 16:9-Firmenmaster hoch. Er bestimmt Markenwelt, Layouts, Schriften und Farben. Erzeugte Dateien werden über ihre ganze Karte direkt heruntergeladen, ohne einen internen Asset-Hinweis. Downloads verändern den Research-Fortschritt nicht.",
      "Every report can be shared directly as a concise **stakeholder PDF** or editable **PowerPoint**. A ten-minute readout normally becomes six to eight decision-led core slides: a decision dashboard, real screens, one persona overview, reaction movement, and a concrete editable revision instead of scattered text findings. The response matrix, method, and sources stay in a short appendix; talk tracks, evidence, caveats, and transitions become native PowerPoint speaker notes. Workspace owners upload a 16:9 company master under **Workspace → Templates**. It owns brand, layouts, fonts, and colors. Generated files download directly from their whole card without an internal asset alert. Downloads never advance the research run.")),
    ("prototype",
