...
PRINCIPLES = [
    ...
    ("syntheses",
     ...
     ("Jeder Report lässt sich direkt als kurze **Stakeholder-PDF** oder als editierbares **PowerPoint** teilen. Das Cover erklärt auch Außenstehenden zuerst Testgegenstand, Zielgruppe und Entscheidungsfrage; danach folgen normalerweise sechs bis acht Kernfolien mit Decision Dashboard, realen Screens, Persona-Sammelfolie, Reaktionsverlauf und editierbarem Änderungsvorschlag. Kurze, eigenständige Headlines und PowerPoint-Autofit verhindern Textüberläufe; leere Master-Platzhalter werden entfernt. Antwortmatrix, Methodik und Quellen bleiben im Anhang, Sprechtext und Belege in echten Sprechernotizen. Workspace-Owner laden unter **Workspace → Vorlagen** einen 16:9-Firmenmaster hoch. Er bestimmt Markenwelt, Layouts, Schriften und Farben. Erzeugte Dateien werden über ihre ganze Karte direkt heruntergeladen, ohne internen Asset-Hinweis. Downloads verändern den Research-Fortschritt nicht.",
      "Every report can be shared directly as a concise **stakeholder PDF** or editable **PowerPoint**. The cover first explains the tested artifact, audience, and decision question for readers outside the project; it is followed by normally six to eight core slides with a decision dashboard, real screens, one persona overview, reaction movement, and an editable revision. Short standalone headlines and native PowerPoint autofit prevent text overflow; empty master placeholders are removed. The response matrix, method, and sources stay in the appendix, while talk tracks and evidence live in native speaker notes. Workspace owners upload a 16:9 company master under **Workspace → Templates**. It owns brand, layouts, fonts, and colors. Generated files download directly from their whole card without an internal asset alert. Downloads never advance the research run.")),
    ("prototype",
