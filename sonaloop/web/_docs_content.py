...
PRINCIPLES = [
    ...
    ("syntheses",
     ...
     ("Jeder Report lässt sich direkt als kurze **Stakeholder-PDF** oder als editierbares **PowerPoint** teilen. Workspace-Owner laden unter **Workspace → Vorlagen** einen 16:9-Firmenmaster hoch. Sonaloop entfernt konkrete Inhaltsfolien, prüft Theme, Schriften, Farben, Layouts und Platzhalter und setzt Report-Inhalte in die jeweils passende native Folienstruktur. Das Deck zeigt verständliche Ergebniskapitel statt interner Engine-Phasen; die vollständigen Quellen bleiben im Report. Der Master bestimmt die Markenwelt; ohne ihn gilt die ruhige Standardvorlage. Downloads bleiben auch verfügbar, solange der zugrunde liegende Research-Run noch arbeitet; sie verändern dessen Fortschritt nicht.",
      "Every report can be shared directly as a concise **stakeholder PDF** or editable **PowerPoint**. Workspace owners upload a 16:9 company master under **Workspace → Templates**. Sonaloop strips concrete content slides, checks its theme, fonts, colors, layouts, and placeholders, and places report content into the closest native slide structure. The deck shows plain-language result chapters instead of internal engine phases; complete sources stay in the report. The master owns the visual identity; without one, the polished default applies. Downloads remain available while the underlying research run is still working and never advance its progress.")),
    ("prototype",
