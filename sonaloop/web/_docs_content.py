...
INSPECTOR_SECTIONS = [
    ...
    ("runs", "play",
     ...
     ("Oben in der Leiste zeigt ein Status-Punkt, ob gerade Studien **aktiv** laufen — er wird "
      ...
      "Report; nach einem Abbruch zeigt derselbe Run wieder denselben Report und den nächsten offenen Abschnitt. Die letzte Verify-Synthese muss als expliziter Input im eingefrorenen Report-Graph stehen; fehlt sie in einem ausgefüllten Alt-Report, bleibt dieser Historie und der geregelte Hand-off erzeugt genau einen aktuellen Ersatz, den Retries wiederverwenden. "
      ...
      "same run returns the same report and next unfinished section. The latest verify synthesis must be an explicit input in the frozen graph; a fully authored older report remains history while the governed hand-off creates exactly one current replacement reused by retries. Critic retries also remain one "
