...
def reaction_preflight_action(project_id: str, store: Store) -> dict[str, Any] | None:
    ...
    This is deliberately a router, not a browser. A URL identifies the target;
    only project-owned, admitted records can satisfy the stimulus gate.
    """
    project = store.get_research_project(project_id) or {}
            ...
            **base,
            ...
            "next_call": {
                ...
                "arguments": {
                    ...
                    "operation_id": "<stable screenshot intent; reuse on retry>",
    ...
    manifests = list(project.get("flow_manifests") or [])
    remote_ids = {str(row.get("id") or "") for row in remote_assets}
    non_remote = [row for row in stimuli
                  if not (row.get("kind") == "asset" and row.get("id") in remote_ids)]
    if remote_assets and not manifests and not non_remote:
        revisions = sorted({str((row.get("admission") or {}).get("target_revision") or "")
                            for row in remote_assets})
        selected_revision = str((remote_assets[-1].get("admission") or {}).get(
            "target_revision") or "")
        selected_assets = [
            row for row in remote_assets
        ...
        return {
            **base,
            ...
            "allowed_tools": ["view_asset", "record_flow_manifest"],
            "admitted_screens": [
    ...
    if not understanding and manifests:
        ...
        return {
            **base,
            ...
            "message": (
                "Inspect every exact screenshot with view_asset, then submit one flat observation "
                "per manifest step. The server builds and validates the nested immutable artifact."
            ),
            "allowed_tools": ["view_asset", "record_manifest_product_understanding"],
            "manifest": {
                ...
                "arguments": {
                    ...
                    "observations": [
                        {"step_index": row.get("index"),
                         "claim": f"<what is visibly present on {row.get('label') or 'this screen'}>"}
    ...
    if not understanding:
        return {
            **base,
            ...
            "allowed_tools": ["view_asset", "brief_product_understanding", "record_product_understanding"],
            "next_call": {"tool": "record_product_understanding",
        ...
        return {
            **base,
            ...
            "next_call": {
                ...
                "arguments": {
                    "project_id": project_id,
                    "persona_ids": ["<existing persona id 1>", "<existing persona id 2>"],
                    "selection_rationale": "<why this independent contrast fits the question>",
